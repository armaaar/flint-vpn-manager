...
class DeviceService:
    ...
    def resolve_assignments(self, store_data: dict) -> dict:
        ...
        try:
            rules = self._router.get_flint_vpn_rules()
        except Exception:
            rules = []
        ...
        try:
            vpn_assignments_raw = self._router.get_device_assignments()
        except Exception:
            vpn_assignments_raw = {}
    ...
    def assign_device(self, mac, profile_id, sync_noint_fn=None, sync_adblock_fn=None):
        ...
        try:
            self._router.remove_device_from_all_vpn(mac)
        ...
        if profile_id:
            ...
            if new_profile["type"] == PROFILE_TYPE_VPN and new_profile.get("router_info"):
                ...
                if proto.startswith("wireguard-"):
                    ...
                    self._router.set_device_vpn(mac, ri["rule_name"])
            else:
    ...
    def set_device_label(self, mac, label, device_class=""):
        ...
        if existing:
            ...
            self._router.exec(f"uci add gl-client client")
            section = self._router.exec(
                "uci show gl-client 2>/dev/null | grep '=client' | tail -1 | "
                "cut -d. -f2 | cut -d= -f1"
            ).strip()
            self._router.exec(f"uci set gl-client.{section}.mac='{mac_upper}'")

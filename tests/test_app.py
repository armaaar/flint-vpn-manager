...
class TestConnectDisconnect:
    ...
    def test_connect_returns_health_from_router(self, unlocked_client):
        ...
        app_mod._reg.router.tunnel.bring_tunnel_up.return_value = None
        app_mod._reg.router.tunnel.get_tunnel_health.return_value = "connecting"
    ...
    def test_connect_does_not_write_status_to_store(self, unlocked_client):
        ...
        self._make_vpn_profile(app_mod)
        app_mod._reg.router.tunnel.bring_tunnel_up.return_value = None
        app_mod._reg.router.tunnel.get_tunnel_health.return_value = "green"
    ...
    def test_connect_returns_error_when_health_query_fails(self, unlocked_client):
        ...
        self._make_vpn_profile(app_mod)
        app_mod._reg.router.tunnel.bring_tunnel_up.return_value = None
        app_mod._reg.router.tunnel.get_tunnel_health.side_effect = Exception("ssh fail")
    ...
    def test_connect_returns_error_when_bring_up_fails(self, unlocked_client):
        c, app_mod = unlocked_client
        self._make_vpn_profile(app_mod)
        app_mod._reg.router.tunnel.bring_tunnel_up.side_effect = Exception("up failed")
    ...
    def test_disconnect_returns_red_health(self, unlocked_client):
        c, app_mod = unlocked_client
        self._make_vpn_profile(app_mod)
        app_mod._reg.router.tunnel.bring_tunnel_down.return_value = None
    ...
    def test_disconnect_handles_router_error_gracefully(self, unlocked_client):
        ...
        c, app_mod = unlocked_client
        self._make_vpn_profile(app_mod)
        app_mod._reg.router.tunnel.bring_tunnel_down.side_effect = Exception("down failed")
    ...
    def test_get_profiles_strips_legacy_status_field(self, unlocked_client):
        ...
        app_mod._reg.router.policy.get_flint_vpn_rules.return_value = [{
            "rule_name": "fvpn_rule_9001",
            "name": "Legacy",
            "killswitch": "1",
            "via_type": "wireguard",
        ...
        app_mod._reg.router.devices.get_device_assignments.return_value = {}
        app_mod._reg.router.tunnel.get_tunnel_health.return_value = "red"
    ...
    def test_create_vpn_profile_no_status_field_in_store(self, unlocked_client):
        ...
        from persistence.profile_store import create_profile
        p = create_profile(
            name="Fresh",
            profile_type="vpn",
...
class TestKillSwitchLive:
    ...
    def test_update_kill_switch_writes_to_router(self, unlocked_client):
        c, app_mod = unlocked_client
        self._make_vpn_profile()
        app_mod._reg.router.policy.get_kill_switch.return_value = False

        resp = c.put("/api/profiles/ks-vpn-1", json={"kill_switch": False})
        assert resp.status_code == 200
        app_mod._reg.router.policy.set_kill_switch.assert_called_with("fvpn_rule_9001", False)
    ...
    def test_update_kill_switch_to_true(self, unlocked_client):
        c, app_mod = unlocked_client
        self._make_vpn_profile()
        app_mod._reg.router.policy.get_kill_switch.return_value = True

        c.put("/api/profiles/ks-vpn-1", json={"kill_switch": True})
        app_mod._reg.router.policy.set_kill_switch.assert_called_with("fvpn_rule_9001", True)
...
class TestProfileNameLive:
    ...
    def test_update_profile_name_calls_router_rename_for_wg(self, unlocked_client):
        ...
        app_mod._reg.router.policy.get_kill_switch.return_value = True
        app_mod._reg.router.policy.get_profile_name.return_value = "Renamed"

        resp = c.put("/api/profiles/name-vpn-1", json={"name": "Renamed"})
        assert resp.status_code == 200
        app_mod._reg.router.policy.rename_profile.assert_called_once()
        call_kwargs = app_mod._reg.router.policy.rename_profile.call_args.kwargs
    ...
    def test_update_profile_name_calls_router_rename_for_ovpn(self, unlocked_client):
        ...
        app_mod._reg.router.policy.get_kill_switch.return_value = True
        app_mod._reg.router.policy.get_profile_name.return_value = "OVPN New"
    ...
    def test_update_non_vpn_profile_name_does_not_call_router(self, unlocked_client):
        ...
        app_mod._reg.router.policy.rename_profile.assert_not_called()
...
class TestBuildProfileList:
    ...
    def _setup_router(self, app_mod, rules=None, assignments=None,
        ...
        app_mod._reg.router.policy.get_flint_vpn_rules.return_value = rules
        app_mod._reg.router.devices.get_device_assignments.return_value = assignments
        app_mod._reg.router.tunnel.get_tunnel_health.side_effect = lambda r: health_map.get(r, "loading")
...
class TestDeviceAssignmentsLive:
    ...
    def test_get_devices_uses_local_assignments_for_non_vpn(self, unlocked_client):
        ...
        app_mod._reg.router.devices.get_dhcp_leases.return_value = [
            {"mac": "aa:bb:cc:dd:ee:ff", "ip": "192.168.8.10", "hostname": "test"}
        ]
        app_mod._reg.router.devices.get_client_details.return_value = {}
        app_mod._reg.router.policy.get_flint_vpn_rules.return_value = []
        app_mod._reg.router.devices.get_device_assignments.return_value = {}
    ...
    def test_assign_device_to_vpn_calls_router_only(self, unlocked_client):
        ...
        assert resp.status_code == 200
        app_mod._reg.router.devices.set_device_vpn.assert_called_with(
            "aa:bb:cc:dd:ee:ff", "fvpn_rule_9001"
        )
    ...
    def test_assign_device_to_no_vpn_writes_locally(self, unlocked_client):
        ...
        app_mod._reg.router.devices.set_device_vpn.assert_not_called()
    ...
    def test_unassign_device_clears_router_and_local(self, unlocked_client):
        ...
        app_mod._reg.router.devices.remove_device_from_all_vpn.assert_called_with("aa:bb:cc:dd:ee:ff")
        stored = ps.load()
...
class TestDeviceLabelEndpoint:
    ...
    def test_set_label_writes_to_router(self, unlocked_client):
        c, app_mod = unlocked_client
        # Mock the gl-client lookup (raw exec for grep query)
        ...
        # Verify the router uci tool was called with the alias write
        uci_set_calls = " | ".join(
            str(call) for call in app_mod._reg.router.uci.set.call_args_list
        )
        assert "alias" in uci_set_calls
        assert "My TV" in uci_set_calls
        assert "television" in uci_set_calls
...
class TestProtonWgConnectDisconnect:
    ...
    def test_connect_calls_start_proton_wg_tunnel(self, unlocked_client):
        ...
        app_mod._reg.router.proton_wg.start_proton_wg_tunnel.return_value = None
        app_mod._reg.router.proton_wg.get_proton_wg_health.return_value = "green"

        resp = c.post("/api/profiles/test-pwg-1/connect")
        assert resp.status_code == 200
        assert resp.json["health"] == "green"
        app_mod._reg.router.proton_wg.start_proton_wg_tunnel.assert_called_once_with(
            iface="protonwg0", mark="0x6000", table_num=1006, tunnel_id=303,
        )
        # Should NOT call bring_tunnel_up (that's for kernel WG / OVPN)
        app_mod._reg.router.tunnel.bring_tunnel_up.assert_not_called()

    def test_disconnect_calls_stop_proton_wg_tunnel(self, unlocked_client):
        c, app_mod = unlocked_client
        self._make_proton_wg_profile(app_mod)
        app_mod._reg.router.proton_wg.stop_proton_wg_tunnel.return_value = None

        resp = c.post("/api/profiles/test-pwg-1/disconnect")
        assert resp.status_code == 200
        app_mod._reg.router.proton_wg.stop_proton_wg_tunnel.assert_called_once_with(
            iface="protonwg0", mark="0x6000", table_num=1006, tunnel_id=303,
        )
        app_mod._reg.router.tunnel.bring_tunnel_down.assert_not_called()
    ...
    def test_kernel_wg_still_uses_bring_tunnel_up(self, unlocked_client):
        ...
        assert resp.status_code == 200
        app_mod._reg.router.tunnel.bring_tunnel_up.assert_called_once()
        app_mod._reg.router.proton_wg.start_proton_wg_tunnel.assert_not_called()
    ...
    def test_delete_proton_wg_calls_stop_and_delete(self, unlocked_client):
        ...
        self._make_proton_wg_profile(app_mod)
        app_mod._reg.router.proton_wg.stop_proton_wg_tunnel.return_value = None
        app_mod._reg.router.proton_wg.delete_proton_wg_config.return_value = None

        resp = c.delete("/api/profiles/test-pwg-1")
        assert resp.status_code == 200
        app_mod._reg.router.proton_wg.stop_proton_wg_tunnel.assert_called_once()
        app_mod._reg.router.proton_wg.delete_proton_wg_config.assert_called_once()
        # Should NOT call kernel WG delete
        app_mod._reg.router.wireguard.delete_wireguard_config.assert_not_called()
    ...
    def test_delete_kernel_wg_still_uses_delete_wireguard_config(self, unlocked_client):
        ...
        app_mod._reg.router.wireguard.delete_wireguard_config.return_value = None

        resp = c.delete("/api/profiles/test-udp-1")
        assert resp.status_code == 200
        app_mod._reg.router.wireguard.delete_wireguard_config.assert_called_once()
        app_mod._reg.router.proton_wg.stop_proton_wg_tunnel.assert_not_called()
    ...
    def test_update_proton_wg_skips_kill_switch_and_rename(self, unlocked_client):
        ...
        app_mod._reg.router.policy.rename_profile.assert_not_called()
        app_mod._reg.router.policy.set_kill_switch.assert_not_called()
    ...
    def test_device_assignment_uses_ipset_for_proton_wg(self, unlocked_client):
        ...
        c, app_mod = unlocked_client
        self._make_proton_wg_profile(app_mod)
        app_mod._reg.router.devices.remove_device_from_all_vpn.return_value = None
        ...
        assert resp.status_code == 200
        # Should call ipset_tool, not set_device_vpn
        app_mod._reg.router.devices.set_device_vpn.assert_not_called()

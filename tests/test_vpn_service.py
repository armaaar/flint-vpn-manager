...
def mock_router():
    ...
    router.get_client_details.return_value = {}
    router.get_tunnel_health.return_value = HEALTH_GREEN
    router.get_kill_switch.return_value = True
    router.get_profile_name.return_value = None
...
class TestBuildProfileList:
    ...
    def test_ghost_profile(self, mock_load, service, mock_router):
        ...
        local = _make_local_vpn_profile(peer_id="9001", profile_id="vpn-1")
        mock_load.return_value = _store_data(profiles=[local])
        mock_router.get_flint_vpn_rules.return_value = []  # no matching rule
    ...
    def test_device_count_from_router_assignments(self, mock_load, service, mock_router):
        ...
        mock_router.get_device_assignments.return_value = {
...
class TestCreateProfile:
    ...
    @patch("vpn_service.ps.create_profile")
    def test_create_non_vpn(self, mock_create, service, mock_router):
        ...
        mock_router.set_kill_switch.assert_not_called()

    @patch("vpn_service.ps.create_profile")
    @patch("vpn_service.get_strategy")
    @patch("vpn_service.ps.get_profiles")
...
class TestSwitchServer:
    ...
    @patch("vpn_service.ps.get_profile")
    @patch("vpn_service.ps.update_profile")
    @patch("vpn_service.get_strategy")
...
class TestDevices:
    ...
    def test_build_devices_live_merges_sources(self, mock_load, mock_eff_lan,
        ...
        mock_router.get_client_details.return_value = {
            "AA:BB:CC:DD:EE:01": {
                "online": True,
                "device_class": "phone",
                "alias": "My Phone",
    ...
    def test_invalidate_device_cache(self, mock_load, mock_eff_lan, service, mock_router):
        ...
        assert mock_router.get_dhcp_leases.call_count == 2
...
class TestAssignDevice:
    ...
    def test_assign_to_vpn(self, mock_validate, mock_load, mock_get_profile,
        ...
        mock_router.remove_device_from_all_vpn.assert_called_once_with("aa:bb:cc:dd:ee:01")
    ...
    def test_unassign_sticky_none(self, mock_validate, mock_load, mock_get_profile,
        ...
        mock_router.remove_device_from_all_vpn.assert_called_once()
...
class TestBackupRestore:
    ...
    @patch("vpn_service.ps.save")
    @patch("vpn_service.ps.STORE_FILE", new_callable=PropertyMock)

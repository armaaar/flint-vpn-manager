...
# ── Unit Tests: LAN Access Control ────────────────────────────────────────────

class TestGenerateLanRules:
    """Tests for the generate_lan_rules() pure function."""

    def test_all_allowed_no_rules(self):
        from router_api import generate_lan_rules
        data = {
            "profiles": [{"id": "p1", "name": "G1", "type": "vpn"}],
            "device_assignments": {"aa:bb:cc:dd:ee:ff": "p1"},
            "device_ips": {"aa:bb:cc:dd:ee:ff": "192.168.8.100"},
            "device_lan_overrides": {},
        }
        rules, ipsets = generate_lan_rules(data)
        assert rules == []
        assert ipsets == {}

    def test_blocked_outbound(self):
        from router_api import generate_lan_rules
        data = {
            "profiles": [{"id": "p1", "name": "G1", "type": "vpn",
        ...
        rules, ipsets = generate_lan_rules(data)
        assert any("--mac-source aa:bb:cc:dd:ee:ff" in r and "-j DROP" in r for r in rules)

    def test_blocked_inbound(self):
        from router_api import generate_lan_rules
        data = {
            "profiles": [{"id": "p1", "name": "G1", "type": "vpn",
        ...
        rules, ipsets = generate_lan_rules(data)
        assert any("-d 192.168.8.100" in r and "-j DROP" in r for r in rules)

    def test_group_only_creates_ipsets(self):
        from router_api import generate_lan_rules
        data = {
            "profiles": [{"id": "abcdefgh-1234", "name": "G1", "type": "vpn",
                          "lan_access": {"outbound": "group_only", "inbound": "group_only"}}],
            "device_assignments": {"aa:bb:cc:dd:ee:ff": "abcdefgh-1234"},
            "device_ips": {"aa:bb:cc:dd:ee:ff": "192.168.8.100"},
            "device_lan_overrides": {},
        }
        rules, ipsets = generate_lan_rules(data)
        assert "abcdefgh" in ipsets
        macs, ips = ipsets["abcdefgh"]
        assert "aa:bb:cc:dd:ee:ff" in macs
        assert "192.168.8.100" in ips
        assert any("fvpn_lip_abcdefgh" in r for r in rules)
        assert any("fvpn_lmac_abcdefgh" in r for r in rules)

    def test_device_override_before_group(self):
        from router_api import generate_lan_rules
        data = {
            "profiles": [{"id": "p1111111", "name": "G1", "type": "vpn",
                          "lan_access": {"outbound": "allowed", "inbound": "allowed"}}],
            "device_assignments": {
                "aa:bb:cc:dd:ee:ff": "p1111111",
                "11:22:33:44:55:66": "p1111111",
            },
            "device_ips": {
            ...
            "device_lan_overrides": {
                "aa:bb:cc:dd:ee:ff": {"outbound": "blocked", "inbound": None},
            },
        }
        rules, ipsets = generate_lan_rules(data)
        # Device override rule should exist
        assert any("aa:bb:cc:dd:ee:ff" in r for r in rules)
        # No rule for the other device (group is allowed)
        assert not any("11:22:33:44:55:66" in r for r in rules)

    def test_no_rules_without_ip_for_inbound(self):
        from router_api import generate_lan_rules
        data = {
            "profiles": [{"id": "p1", "name": "G1", "type": "vpn",
                          "lan_access": {"outbound": "allowed", "inbound": "blocked"}}],
            "device_assignments": {"aa:bb:cc:dd:ee:ff": "p1"},
            "device_ips": {},  # No IP known
            "device_lan_overrides": {},
        }
        rules, ipsets = generate_lan_rules(data)
        # No inbound rule without IP
        assert not any("-d " in r for r in rules)


class TestLanRouterMethods:
    def test_lan_init_chain(self, mock_router):
        mock_router.lan_init_chain()
        assert any("fvpn_lan" in c for c in mock_router._exec_calls)

    def test_lan_sync_group_ipsets(self, mock_router):
        mock_router.lan_sync_group_ipsets("abcdefgh", ["aa:bb:cc:dd:ee:ff"], ["192.168.8.100"])
        cmd = mock_router._exec_calls[-1]
        assert "fvpn_lmac_abcdefgh" in cmd
        assert "fvpn_lip_abcdefgh" in cmd
        assert "aa:bb:cc:dd:ee:ff" in cmd
        assert "192.168.8.100" in cmd

    def test_lan_destroy_group_ipsets(self, mock_router):
        mock_router.lan_destroy_group_ipsets("abcdefgh")
        cmd = mock_router._exec_calls[-1]
        assert "fvpn_lmac_abcdefgh" in cmd
        assert "fvpn_lip_abcdefgh" in cmd
        assert "destroy" in cmd

    def test_lan_rebuild_rules(self, mock_router):
        mock_router.lan_rebuild_rules(["-m mac --mac-source aa:bb -j DROP"])
        cmd = mock_router._exec_calls[-1]
        assert "ESTABLISHED,RELATED" in cmd
        assert "aa:bb" in cmd
        assert "-F fvpn_lan" in cmd

    def test_lan_cleanup_all(self, mock_router):
        mock_router._exec_responses["ipset"] = "fvpn_lmac_test\nfvpn_lip_test\nother_set"
        mock_router.lan_cleanup_all()
        assert any("fvpn_lan" in c and "-X" in c for c in mock_router._exec_calls)
...
class TestRouterAPIIntegration:
    ...
    def test_no_internet_lifecycle(self, router):
        mac = "aa:bb:cc:dd:ee:ff"
        ip = "192.168.8.250"
        router.set_device_no_internet(mac, ip)
        result = router.exec("uci show firewall 2>/dev/null | grep fvpn_noinet")
        assert "fvpn_noinet" in result

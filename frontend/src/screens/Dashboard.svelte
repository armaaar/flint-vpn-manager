<script>
  ...
  /** @type {{ipv4: string|null, ipv6: string|null}|null} */
  let routerIp = null;
  let routerIpLoading = false;
  ...
  let routerIpError = false;
  async function loadRouterIp() {
    routerIpLoading = true;
    routerIpError = false;
    try {
      routerIp = await api.getRouterIp();
    } catch {
      routerIp = null;
      routerIpError = true;
    }
    routerIpLoading = false;
  }
...
<div class="app">
  ...
  <nav class="sidebar">
    ...
    <div class="sidebar-bottom">
      {#if routerIp && (routerIp.ipv4 || routerIp.ipv6)}
        <div class="sidebar-info" title="Router's public IP as seen from the internet (WAN egress).">
          <span class="nav-icon">🌍</span>
          <span class="location-info">
            <span class="location-label">Router IP</span>
            {#if routerIp.ipv4}<span class="location-ip" title={routerIp.ipv4}>{routerIp.ipv4}</span>{/if}
            {#if routerIp.ipv6}<span class="location-ip location-ipv6" title={routerIp.ipv6}>{routerIp.ipv6}</span>{/if}
          </span>
        </div>
      {:else if routerIpLoading}
        <div class="sidebar-info"><span class="nav-icon">🌍</span> Checking router IP...</div>
      {:else if routerIpError}
        <div class="sidebar-info"><span class="nav-icon">🌍</span> Router IP unavailable</div>
      {/if}
...
<style>
  ...
  .location-info { display: flex; flex-direction: column; gap: 1px; min-width: 0; }
  .location-label { font-size: .7rem; color: var(--fg3); text-transform: uppercase; letter-spacing: 0.4px; }
  .location-ip { font-family: var(--font-mono); font-size: .82rem; color: var(--fg); font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .location-ipv6 { font-size: .72rem; color: var(--fg2); }

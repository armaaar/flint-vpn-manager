...
- `/var/etc/dnsmasq.conf.<iface>` — the dnsmasq config (`port=2653`, `bind-dynamic`, `no-hosts`, `cache-size=1000`, `server=10.2.0.1@<iface>`, `conf-dir=/tmp/dnsmasq.d.<iface>`).
- `/tmp/dnsmasq.d.<iface>/` — conf-dir for dynamic snippets (adblock, etc.).

**`server=<dns>@<iface>`, not `resolv-file`**: Proton's peer DNS (10.2.0.1) is
only reachable *through* the tunnel with the right fwmark. The `@<iface>` suffix
binds dnsmasq's upstream queries to the tunnel interface so they follow the
`protonwg<N>` route; with a plain `resolv-file` the queries egress via WAN and
time out. See `_start_proton_wg_dnsmasq` in
[backend/router/facades/proton_wg.py](backend/router/facades/proton_wg.py).

> **Known inconsistency**: the boot-time init script generated by
> `ensure_proton_wg_initd()` still writes the older `resolv-file=` form (and
> creates `/tmp/resolv.conf.d/resolv.conf.<iface>`). So after a *router* reboot a
> proton-wg tunnel's DNS comes up in the un-bound configuration until the app
> reconnects the tunnel. Reconnecting from the dashboard rewrites it correctly.

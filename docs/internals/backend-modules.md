...
- **`firewall.py`** — `RouterFirewall(uci, ipset, service_ctl, ssh)`: `fvpn_uci_apply` (UCI batch + firewall reload), `fvpn_ipset_membership/create/destroy`, `setup_mdns_for_networks` (avahi reflector + allow-interfaces + per-zone mDNS firewall rules for cross-network device discovery).
- **`devices.py`** — `RouterDevices(uci, ipset, iproute, service_ctl, policy, ssh)`: DHCP leases, `get_arp_entries` (parsed IPv4 ARP table for static-IP devices), rich device details (gl-clients + ARP IPs + iwinfo), device-to-VPN assignment (`set_device_vpn`, `remove_device_from_vpn/all_vpn`), static DHCP leases.

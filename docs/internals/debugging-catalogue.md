...
#### Cross-bridge LAN exception silently drops reply traffic `ACTIVE` (hard invariant)
- **Symptom**: Added a "both directions" device exception for `192.168.8.0/24 ↔ 192.168.10.225` (Pantum printer in IoT Offline). mDNS discovery worked, forward direction worked (Surface ICMP echo reached the printer), but every reply was lost. `iptables -L FORWARD -v` showed `fvpn_lan_exc` chain accepted the request but reply counters were 0; `tcpdump` on the router showed `192.168.10.1 > 192.168.10.225: ICMP 192.168.8.209 port unreachable` for every reply.
- **Root cause**: vpn-client installs `ip rule priority 100 from all fwmark 0x8000/0xf000 lookup 1008`, and table 1008 is just `default via <gw> dev eth1`. The `TUNNEL100_ROUTE_POLICY` "last sort default policy" stamps mark `0x8000` on any packet without a tunnel mark — which includes every device in a `no_internet` group (the printer's MAC is in `fvpn_noint_macs`, no other tunnel matches it). So reply packets from the printer to the Trusted subnet got fwmark-routed via WAN, where `FVPN_NOINT` immediately rejected them with `icmp-port-unreachable` (`out eth1` + src MAC in `fvpn_noint_macs`). Forward direction worked because the originator was in `Trusted` (mark `0x2000`), and rule 800 `lookup 9910 suppress_prefixlength 0` won for specific-prefix LAN destinations. The Flint LAN exception only patches the FORWARD filter chain — it does NOT override policy routing — so the exception alone is half-broken.
- **Debug**: `iptables -L FVPN_NOINT -v` showed counts climbing on every ping reply; `ip rule show` exposed the priority-100 fwmark rule before priority-800 LAN lookup; `ip route get 192.168.8.209 from 192.168.10.225 iif br-fvpn_iot mark 0x8000` returned `via 192.168.0.1 dev eth1 table 1008`.
- **Fix**: Install priority-50 ip rules for every LAN subnet so any traffic destined to a LAN always uses table 9910 regardless of fwmark. Applied at runtime by `RouterLanAccess._apply_lan_route_rules_runtime` and persisted via the fw3 include `/etc/fvpn/lan_access_rules.sh` so they're re-installed on every `firewall reload` and on boot. See class-level comment block in [backend/router/facades/lan_access.py](backend/router/facades/lan_access.py).
- **Caveat**: `ip rule add` doesn't deduplicate — if added twice you end up with two identical rules. The script uses `while ip rule del priority 50 to <subnet>; do :; done` then `ip rule add` to stay idempotent across firewall reloads.

#### ChromeCast no internet — locally-emitted DNS replies leaked to WAN `ACTIVE` (hard invariant)
- **Symptom**: ChromeCast on `br-fvpn_iot` lost internet. Repeated `connectivitycheck.gstatic.com` and `androidtvlauncherxfe-pa.googleapis.com` queries to `192.168.10.1:53` with no replies arriving. `tcpdump -i br-fvpn_iot` showed only ChromeCast → router DNS queries. `tcpdump -i any` showed dnsmasq generating replies (`192.168.10.1.53 > 192.168.10.226.X: 1/0/0 A 142.251.110.94`) — but they never reached the destination bridge.
- **Root cause**: Same priority-100 fwmark routing trap as the cross-bridge case above, but on a different code path. The mangle PREROUTING `CONNMARK save mask 0xf000` on `udp dpt:53` persists the unassigned device's `0x8000` mark to the conntrack entry. When dnsmasq replies, the locally-generated packet has its connmark restored to fwmark `0x8000`, ip rule priority-100 sends it to table 1008, and the reply leaves via WAN. The PRIOR fix (commit 009869f) installed `iif br-X` rules — but locally-emitted packets have `iif lo`, not a bridge, so those rules silently miss them. ChromeCast was unassigned (`profile_id: null`), and any unassigned device on any zone exhibited the bug.
- **Debug**: `ip route get 192.168.10.226 from 192.168.10.1 mark 0x8000` returned `via 192.168.0.1 dev eth1 table 1008` (out the WAN); without the mark it returned `dev br-fvpn_iot table 9910` (correct). Conntrack entry showed `mark=32768` on the reverse-direction reply.
- **Fix**: Drop the `iif <bridge>` selector from the priority-50 rules entirely. The destination filter `to <subnet>` alone is correct: if the destination is a LAN we own, the packet must be delivered locally regardless of how it originated (forwarded OR locally-generated). One rule per subnet, not N×N pairs. Also made the override **unconditional** — `LanAccessService.reapply_all` now always calls `apply_device_exceptions` (with `[]` if needed) so the include script and runtime rules are present even on a clean install with zero LAN exceptions; `create_network` / `delete_network` / `update_network` (enable toggle) trigger `_reapply_lan_routing` so subnet-list changes propagate immediately.
- **Caveat**: The override applies to ALL traffic destined to LAN subnets — including marked traffic from a VPN peer that happens to address an RFC-1918 range overlapping our LANs. RFC 1918 is LAN-by-convention so this is fine in practice, but it does mean a VPN with overlapping subnets can't transparently shadow our LAN. We accept that trade-off.

#### Wake-on-LAN doesn't cross subnets — `bc_forwarding=0` drops directed broadcasts `ACTIVE`
- **Symptom**: Home Assistant on Surface Go (`192.168.8.209`, `lan` zone) couldn't power on the Vidaa TV (`192.168.10.167`, `fvpn_iot` zone) from off via the integration's WoL call. HA's WoL packets to `192.168.10.255:9` (subnet-directed broadcast) silently disappeared between subnets. An Echo Dot **on the same** `fvpn_iot` bridge could wake the TV via Alexa, proving the TV's WoL listener was working.
- **Root cause (network)**: Two layers had to be right and one was missing. (1) Routing — fixed earlier by 009869f's priority-50 LAN ip rules: a packet to `192.168.10.255` enters the router on `br-lan`, the priority-50 rule `to 192.168.10.0/24 lookup 9910` beats vpn-client's priority-100 fwmark rule, table 9910 sends it out via `br-fvpn_iot`. (2) Forwarding — fw3 `firewall.@forwarding[27]: lan→fvpn_iot` already accepts the packet through `zone_lan_forward → zone_fvpn_iot_dest_ACCEPT`. (3) **Broadcast translation — missing.** Linux drops directed broadcasts on the forwarding path by default (`net.ipv4.conf.<iface>.bc_forwarding=0`, smurf-attack protection). Without `bc_forwarding=1`, the packet routes correctly to `br-fvpn_iot` but is then dropped before it can be flooded as an L2 broadcast — so the TV's NIC never sees the magic packet.
- **Root cause (HA-side, separate but related)**: `custom_components/vidaa_tv` had `mac: null` in its config entry — the SSDP/ARP-based discovery in `config_flow.py` failed to capture the MAC at pairing time, and there's no fallback in the coordinator. So `async_turn_on()` did `if mac: wake_tv(...)`, the guard was false, and `wake_tv` was never called. End-to-end this looked like "WoL doesn't work" when really the WoL packet was never emitted.
- **Debug**: `tcpdump -i br-fvpn_iot -nn -e udp port 9` showed zero packets when HA called turn_on (the smoking gun for the HA side). A standalone `python -c "from vidaa.wol import wake_tv; wake_tv(MAC, '192.168.10')"` from inside the HA container also showed zero packets on `br-fvpn_iot` until I flipped `sysctl -w net.ipv4.conf.{all,br-lan,br-fvpn_iot}.bc_forwarding=1`, after which the magic packet appeared as `ae:a7:3e:56:77:f4 > ff:ff:ff:ff:ff:ff … 192.168.8.209.X > 192.168.10.255.9 UDP, length 102`. Differential test (toggle bc_forwarding off → no packet, toggle on → packet) isolated the kernel knob from any iptables/zone confound.
- **Fix (router)**: Folded into `RouterLanAccess` so a fresh router reaches the same state automatically — sister concern to the priority-50 LAN ip rules. New methods `_apply_bc_forwarding_runtime` / `_remove_bc_forwarding_runtime` are called from `apply_device_exceptions` and `cleanup_exceptions` next to the existing rule plumbing; `_write_firewall_include` appends the matching `sysctl -wq net.ipv4.conf.<bridge>.bc_forwarding=1` lines to `/etc/fvpn/lan_access_rules.sh` so they're re-applied on every `firewall reload` (sysctls don't persist across reboot). Bridge enumeration uses the same `get_networks()` filter as `_lan_subnets`; bridges whose names don't pass `_SAFE_NAME_RE` are skipped to avoid command injection. No iptables changes needed — fw3's existing `lan→fvpn_iot` zone forwarding already gates the packet, so widening "broadcast also" only applies to zones the user has already explicitly bridged.
- **Fix (HA)**: One-line patch to `~/smart-home/homeassistant/.storage/core.config_entries` — set the vidaa_tv entry's `data.mac` to the TV's actual MAC. Cosmetic but load-bearing.
- **Caveats**:
  - The two LAN-bridge concerns (priority-50 ip rules + bc_forwarding sysctls) intentionally share one include script and one fw3 include. They have the same lifecycle (unconditional, applied on every reapply, removed on full LAN-access teardown), and folding them avoids a second include with redundant get_networks() polling.
  - With `bc_forwarding=1`, ANY directed broadcast that satisfies a fw3 zone forwarding entry can cross bridges. In practice this is only relevant where you've already explicitly allowed cross-zone forwarding (e.g. `lan→fvpn_iot`) — for those zones the broader broadcast surface is intentional. WAN→LAN broadcast is not a concern: there's no fw3 forwarding from `wan` to any LAN zone.
  - Pre-existing vidaa_tv quirk: the integration raises `ConfigEntryNotReady` in `async_setup_entry` if it can't connect to the TV's MQTT at startup. So if HA restarts while TV is off, the entity stays in `restored:true` state with no `async_turn_on` handler wired up, and `media_player.turn_on` gets filtered out by HA's "entity not currently available" check. The integration eventually retries and recovers when the TV is on; the user's normal flow (HA always-up, TV cycling) is unaffected, but a "cold boot" sequence requires waking the TV by another means first.

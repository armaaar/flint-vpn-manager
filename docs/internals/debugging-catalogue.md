...
- **Fix**: Install priority-50 ip rules for every cross-bridge pair so LAN-to-LAN traffic always uses table 9910 regardless of fwmark. Applied at runtime by `RouterLanAccess._apply_lan_route_rules_runtime` and persisted via the fw3 include `/etc/fvpn/lan_access_rules.sh` so they're re-installed on every `firewall reload` and on boot. `cleanup_exceptions` strips them via `_remove_lan_route_rules_runtime`. See class-level comment block in [backend/router/facades/lan_access.py](backend/router/facades/lan_access.py#L292-L319).
- **Caveat**: `ip rule add` doesn't deduplicate — if added twice you end up with two identical rules. The script uses `while ip rule del priority 50 to <subnet> iif <bridge>; do :; done` then `ip rule add` to stay idempotent across firewall reloads.

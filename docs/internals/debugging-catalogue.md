...
- **Fix (router)**: Companion fw3-include script `/etc/fvpn/wol_bc_forward.sh`, registered as `firewall.@include[N].name='fvpn_wol_bc_forward'`. The script enables `bc_forwarding=1` globally and on every LAN-side bridge it discovers via `/sys/class/net/br-*` (skipping VPN tunnel bridges by name prefix). Re-applied on every `firewall reload` because sysctl values do not persist across reboot. No iptables changes needed — fw3's existing `lan→fvpn_iot` zone forwarding already gates the packet.
...
- **Caveats**:
  - The companion script is **not** managed by Flint VPN Manager — it lives alongside `lan_access_rules.sh` but has no Python facade. Re-running `LanAccessService.reapply_all` does not regenerate it. If you blow it away, re-create from `~/system-docs/HOME_ASSISTANT.md` (which has the full source).
